    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        if (result.status === 'failure') {
          const msg = Message.format({ status: 'failure', task_id: task.id, action_type: 'task', comments: result.comments, content: result.comments, json: { comments: result.comments } })
          this.onTokenStream(msg);
          await Message.saveToDB(msg, this.context.conversation_id);
          return;
        }
